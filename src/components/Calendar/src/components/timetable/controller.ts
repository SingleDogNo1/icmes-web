...
function multiRange(selectDates: { start?: string; end?: string }[], date: string) {
  ...
  const searchResult = selects.some((selectItem: any, index: number) => {
    ...
    if (start && end) {
      if (
        date2timeStamp(start) < date2timeStamp(date) &&
        date2timeStamp(date) < date2timeStamp(end)
      ) {
        return true;
      }
      ...
      if (selects.length > 1) {
        isIncludeOtherRange = selects.some((item: any, i: number) => {
          ...
          if (
            date2timeStamp(selectItemDate.start) < date2timeStamp(prevItemDate) &&
            date2timeStamp(prevItemDate) < date2timeStamp(selectItemDate.end)
          ) {
            return true;
          }
        });
...
export { singleSelect, multiSelect, rangeSelect, multiRange };
