...
function rangeOption({ selectDate, date }: any) {
  ...
  if (start === date) {
    ...
    return 'day-selected selected-range-start' + notCompleteClassName;
  }
  if (end === date) {
    return 'day-selected selected-range-end';
  }
  ...
  if (start && end && date) {
    ...
    if (startTimeStamp < currentTimeStamp && currentTimeStamp < endTimeStamp) {
      return 'day-selected selected-range-includes';
    }
  }
}
...
function multiRangeOption({ selectDate = [], date }: any) {
  ...
  selectDate.some((selectItem: any) => {
    ...
    if (start === date) {
      const notCompleteClassName = end ? '' : ' selected-range-not-complete';
      className = 'day-selected selected-range-start' + notCompleteClassName;
      return true;
    }
    if (end === date) {
      className = 'day-selected selected-range-end';
      return true;
    }
    ...
    if (start && end && date) {
      ...
      if (startTimeStamp < currentTimeStamp && currentTimeStamp < endTimeStamp) {
        className = 'day-selected selected-range-includes';
        return true;
      }
    }
  });
...
function multiOption({ selectDate, date }: any) {
  return selectDate.includes(date) ? 'day-selected' : undefined;
}

function selectOption({ date, selectDate }: any) {
  return selectDate === date ? 'day-selected' : undefined;
}
